...
# Register Feedback Blueprint (FUT)
try:
    from feedback_routes import feedback_bp
    app.register_blueprint(feedback_bp, url_prefix='/api/feedback')
    logger.info("✅ Feedback Blueprint registered successfully")
except ImportError as e:
    logger.warning(f"⚠️ Feedback Blueprint not available: {e}")


# Import and register administrator routes
try:
    from routes.administrator_routes import admin_bp, init_admin_routes
    
    # Database configuration for admin routes
    admin_db_config = {
    ...
    logger.error(f"❌ Failed to import administrator routes: {e}")
...
if __name__ == '__main__':
    ...
    # Get port from environment variable or default to 5006
    ...
    if enhanced_jd_available:
        print("\nEnhanced JD Processing Endpoints Available")
    
    if enhanced_analytics_available:
        print("\nEnhanced Analytics Endpoints Available")
    
    print("="*60 + "\n")

    # Add route for serving uploads
