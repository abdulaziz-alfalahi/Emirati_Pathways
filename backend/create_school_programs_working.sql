-- School Programs Database Schema for PostgreSQL (Working Version)
-- Simplified version without complex constraints that might cause issues
-- Built incrementally to ensure each part works
-- Created: 2025-09-27

-- Enable UUID extension if not already enabled
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing school_programs table to start fresh
...
CREATE TABLE program_success_metrics (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE program_workflow_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    program_id UUID NOT NULL REFERENCES school_programs(id) ON DELETE CASCADE,
...
CREATE INDEX idx_school_programs_category ON school_programs(category);
CREATE INDEX idx_school_programs_status ON school_programs(status);
CREATE INDEX idx_school_programs_workflow_stage ON school_programs(workflow_stage);
CREATE INDEX idx_school_programs_featured ON school_programs(featured);
CREATE INDEX idx_school_programs_age_range ON school_programs(target_age_min, target_age_max);
CREATE INDEX idx_school_programs_dates ON school_programs(start_date, end_date);

CREATE INDEX idx_program_workflow_history_program ON program_workflow_history(program_id);
CREATE INDEX idx_program_workflow_history_actor ON program_workflow_history(actor_id);
CREATE INDEX idx_program_workflow_history_stage ON program_workflow_history(stage_to);

CREATE INDEX idx_program_reviews_program ON program_reviews(program_id);
CREATE INDEX idx_program_reviews_reviewer ON program_reviews(reviewer_id);
CREATE INDEX idx_program_reviews_status ON program_reviews(status);
CREATE INDEX idx_program_reviews_type ON program_reviews(review_type);

CREATE INDEX idx_program_tags_program ON program_tags(program_id);
CREATE INDEX idx_program_tags_name ON program_tags(tag_name);
...
CREATE INDEX idx_program_notifications_recipient ON program_notifications(recipient_id);
CREATE INDEX idx_program_notifications_unread ON program_notifications(is_read) WHERE is_read = FALSE;
