    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                ...
                """,
                (
                    job_id, current_user_id, request.form.get('document_type'), filename,
                    stored_name, file.mimetype, request.content_length or 0, storage_path
                )
            )
    ...
    try:
        ...
        try:
            ...
            for c in candidates:
                ...
                score = CandidateSearchEngine.calculate_match_score.__func__(None, c, job_requirements)  # staticmethod call pattern
                matched.append({
                    'candidate_id': c['id'],
...
def get_shortlist(job_id):
    ...
    try:
        ...
        try:
            cursor.execute("""
                SELECT js.candidate_id, u.first_name, u.last_name, u.email, js.notes, js.created_at
                FROM job_shortlists js
                INNER JOIN job_postings jp ON js.job_posting_id = jp.id
                INNER JOIN hr_profiles hp ON jp.company_id = hp.company_id
                INNER JOIN users u ON js.candidate_id = u.id
                WHERE js.job_posting_id = %s AND hp.user_id = %s
    ...
    try:
        ...
        try:
            ...
            cursor.execute(
                ...
                VALUES (%s, %s, %s, %s)
                ON CONFLICT (job_posting_id, candidate_id) DO UPDATE SET notes = EXCLUDED.notes
                RETURNING *
                """,
                (job_id, candidate_id, current_user_id, data.get('notes'))
            )
...
def remove_from_shortlist(job_id, candidate_id):
    ...
    try:
        ...
        try:
            cursor.execute("""
                DELETE FROM job_shortlists js USING job_postings jp, hr_profiles hp
                WHERE js.job_posting_id = jp.id AND jp.company_id = hp.company_id
                  AND js.job_posting_id = %s AND js.candidate_id = %s AND hp.user_id = %s
                RETURNING js.*
            """, (job_id, candidate_id, current_user_id))
