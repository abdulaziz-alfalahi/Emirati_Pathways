...
@hr_candidate_search_bp.route('/<int:candidate_id>', methods=['GET'])
@jwt_required()
def get_candidate_details(candidate_id):
    """Get detailed information for a specific candidate (for recruiters)"""
    try:
        # Verify user is HR/Recruiter
        claims = get_jwt()
        user_role = claims.get('role', '')
        
        if user_role not in ['hr', 'recruiter', 'admin']:
            return jsonify({
                'success': False,
                'message': 'Access denied. HR/Recruiter role required.'
            }), 403
        ...
        try:
            ...
            if candidate_data.get('skills'):
                if isinstance(candidate_data['skills'], str):
                    try:
                        ...
                        candidate_data['skills'] = candidate_data['skills'].split(',') if ',' in candidate_data['skills'] else [candidate_data['skills']]
            
            # Format dates
            if candidate_data.get('registered_at'):
                candidate_data['registered_at'] = candidate_data['registered_at'].isoformat() if hasattr(candidate_data['registered_at'], 'isoformat') else str(candidate_data['registered_at'])
            if candidate_data.get('last_login'):
                candidate_data['last_login'] = candidate_data['last_login'].isoformat() if hasattr(candidate_data['last_login'], 'isoformat') else str(candidate_data['last_login'])
            if candidate_data.get('last_application_date'):
                candidate_data['last_application_date'] = candidate_data['last_application_date'].isoformat() if hasattr(candidate_data['last_application_date'], 'isoformat') else str(candidate_data['last_application_date'])
            ...
            for app in cursor.fetchall():
                app_dict = dict(app)
                if app_dict.get('submitted_at'):
                    app_dict['submitted_at'] = app_dict['submitted_at'].isoformat() if hasattr(app_dict['submitted_at'], 'isoformat') else str(app_dict['submitted_at'])
                if app_dict.get('updated_at'):
                    app_dict['updated_at'] = app_dict['updated_at'].isoformat() if hasattr(app_dict['updated_at'], 'isoformat') else str(app_dict['updated_at'])
        ...
        logger.error(f"Error getting candidate details for ID {candidate_id}: {str(e)}")
        return jsonify({
            'success': False,
            'message': f'Error retrieving candidate details: {str(e)}'
        }), 500
