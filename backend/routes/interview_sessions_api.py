    ...
    try:
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                try:
                    # 1. A finished call settles immediately, judged by how
                    #    long anyone was actually in it: the owner's rule is
                    #    that under two minutes was not an interview
                    #    (feedback fb_1785825926). Attendance times come from
                    #    migration 049; the previous version of this query
                    #    joined video_interview_sessions.interview_id, a column
                    #    that does not exist on a table that is never written
                    #    to — it raised every time and silently skipped the
                    #    whole sweep.
                    cur.execute("""
                        UPDATE interview_schedules
                        SET status = CASE
                                WHEN ended_at - started_at >= INTERVAL '2 minutes' THEN 'completed'
                                ELSE 'no_show'
                            END,
                            updated_at = CURRENT_TIMESTAMP
                        WHERE (candidate_id::text = %s OR recruiter_id::text = %s)
                          AND started_at IS NOT NULL AND ended_at IS NOT NULL
                          AND status IN ('in_progress','joined','started')
                    ...
                    #    Accepting is intent; only joining long enough counts.
                    cur.execute("""
                        UPDATE interview_schedules
                        SET status = CASE
                                WHEN started_at IS NOT NULL
                                     AND COALESCE(ended_at, NOW()) - started_at >= INTERVAL '2 minutes'
                                     THEN 'completed'
