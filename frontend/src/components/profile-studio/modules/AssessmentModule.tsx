...
export const AssessmentModule: React.FC<AssessmentModuleProps> = ({ profile }) => {
    ...
    const getScoreColor = (score: number, max: number) => {
        ...
        if (percentage >= 75) return 'text-teal-600 bg-teal-50 border-teal-200';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            {/* Header */}
            <div>
                <h2 className="text-2xl font-bold text-gray-900">{t('Skills & Assessments', 'المهارات والتقييمات')}</h2>
                <p className="text-gray-500">{t('Validated competencies and D33 strategic alignment.', 'الكفاءات المعتمدة والتوافق الاستراتيجي مع أجندة D33.')}</p>
            </div>
                ...
                <div className="bg-gradient-to-r from-indigo-900 to-purple-900 rounded-xl p-6 text-white shadow-lg relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <h3 className="text-xl font-bold">{t('D33 Strategic Alignment', 'التوافق الاستراتيجي مع D33')}</h3>
                        </div>
                        <p className="text-indigo-200 mb-6 max-w-2xl">
                            {t('Your profile exhibits strong alignment with Dubai\'s D33 Economic Agenda sectors. You are identified as high-potential talent for key growth industries.', 'يُظهر ملفك توافقاً قوياً مع قطاعات أجندة دبي الاقتصادية D33. تم تحديدك كموهبة عالية الإمكانات في الصناعات الرائدة.')}
                        </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {culturalAssessments.length > 0 ? (
                    <div className="lg:col-span-1 bg-white border border-yellow-100 rounded-xl shadow-sm p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            <div className="flex items-center gap-2 mb-4 text-emerald-800">
                                ...
                                <h3 className="font-bold text-lg">{t('National Identity', 'الهوية الوطنية')}</h3>
                            </div>
                                ...
                                <div key={test.id} className="text-center py-6">
                                    <div className="inline-flex items-center justify-center w-32 h-32 rounded-full border-4 border-emerald-100 bg-emerald-50 mb-4">
                                        <div className="text-center">
                                            ...
                                            <span className="text-xs text-emerald-600">{t('out of', 'من')} {test.max_score}</span>
                                        </div>
                                    </div>
                                    <h4 className="font-semibold text-gray-900 mb-1">{test.title}</h4>
                                    <p className="text-sm text-gray-500">{t('High Proficiency', 'إتقان عالٍ')}</p>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-1 bg-gray-50 border border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="font-medium text-gray-900">{t('Measurement Pending', 'في انتظار القياس')}</h3>
                        <p className="text-sm text-gray-500 mt-1">{t('Take the UAE Cultural Competency assessment to verify your national identity alignment.', 'أكمل تقييم الكفاءة الثقافية الإماراتية للتحقق من توافقك مع الهوية الوطنية.')}</p>
                        <button className="mt-4 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
                            {t('Take Assessment', 'ابدأ التقييم')}
                        </button>
                    </div>
                )}
                ...
                <div className="lg:col-span-2 space-y-4">
                    <h3 className="font-bold text-gray-900 flex items-center gap-2">
                        ...
                        {t('Verified Competencies', 'الكفاءات الموثقة')}
                    </h3>

                    {technicalAssessments.length === 0 && otherAssessments.length === 0 && (
                        <div className="p-8 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200">
                            <p className="text-gray-500">{t('No technical assessments completed yet.', 'لم يتم إكمال أي تقييمات تقنية بعد.')}</p>
                        </div>
                    )}
                            ...
                            <div className="flex-1">
                                <div className="flex items-center gap-2">
                                    ...
                                    {test.assessment_type === 'technical' && (
                                        <span className="px-2 py-0.5 bg-teal-100 text-teal-700 text-xs rounded-full">{t('Technical', 'تقني')}</span>
                                    )}
                                </div>
                                <p className="text-xs text-gray-500 mt-1">{t('Completed on', 'تم الإكمال في')} {new Date(test.completed_at || '').toLocaleDateString(isRTL ? 'ar-AE' : 'en-US')}</p>
                            </div>
