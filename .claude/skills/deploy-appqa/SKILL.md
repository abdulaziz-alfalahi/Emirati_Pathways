...
- **New frontend npm dependency**: `git pull` alone is not enough — the Vite dev server must resolve the package from `node_modules`, but APPQA runs **node 12** so a full `npm install` FAILS (unsupported-engine on transitive deps). For a static package (e.g. a @fontsource font: CSS + woff2, no build step), copy just that package dir into `~/Emirati_Pathways/frontend/node_modules/<pkg>` (tar+scp from your local node_modules), then **restart Vite** so it re-optimizes deps: `rm -rf node_modules/.vite`, kill the detached `npm run dev` (PPID 1, no supervisor — find via `pgrep -f 'npm run dev'`), relaunch `setsid nohup npm run dev > vite.log 2>&1 < /dev/null &`, and poll `ss -tln | grep :8089` before trusting it. Verify the public site returns 200 after.
